...
public class Mob extends Entity
{
	...
	private PlayerHolder hold;
	private int animation = 0;
	
	protected Mob(int x, int y)
	{
		hold = new PlayerHolder();
		this.x = x;
		this.y = y;
	}
	...
	public void render(Screen screen)
	{
		...
		if(dir == 0) 
		{
			setSprite(hold.player_forward);
			//sprite = Sprite.player_forward;
			if(getWalking())
			{
				if (getAnimation() % 20 > 10)
				{
					setSprite(hold.player_forward_1);
				}
				else
				{
					flip = 1;
		...
		if(dir == 1) 
		{
			setSprite(hold.player_side);
			if(walking)
			{
				if (animation % 20 > 10)
				{
					setSprite(hold.player_side_1);
				}
				else
				{
					setSprite(hold.player_side_2);
				}
			}
		}
		if(dir == 2) 
		{
			setSprite(hold.player_backward);
			if(walking)
			{
				if (animation % 20 > 10)
				{
					setSprite(hold.player_backward_1);
				}
				else
				{
					flip = 1;
		...
		if (dir == 3)
		{
			setSprite(hold.player_side); //player sprite faces left
			if(walking)
			{
				if (animation % 20 > 10)
				{
