...
public abstract class Level
{
	...
	void render(int xScroll, int yScroll, Screen screen)
	{
		...
		if(xScroll > ((width << 4) - screen.getPhold().getDimension().getWidth())) xScroll = ((width << 4) - screen.getPhold().getDimension().getWidth());
		if(yScroll > ((height << 4) - screen.getPhold().getDimension().getHeight())) yScroll = ((height << 4) - screen.getPhold().getDimension().getHeight());
		screen.setOffset(xScroll, yScroll);
		//This is kind of bad since we are under assumption tiles are only 16
		int x0 = xScroll >> 4;
		int x1 = (xScroll + screen.getPhold().getDimension().getWidth() + 16) >> 4;
		int y0 = yScroll >> 4;
		int y1 = (yScroll + screen.getPhold().getDimension().getHeight() + 16) >> 4;
